...
export function CodeComparison({
  ...
  const beforeCodeText = getTextContent(beforeCode);
  const afterCodeText = getTextContent(afterCode);
  ...
  useEffect(() => {
    async function highlightCode() {
      try {
        ...
        const before = await codeToHtml(beforeCodeText, {
          lang: language,
          theme: selectedTheme,
          transformers: [
        ...
        const after = await codeToHtml(afterCodeText, {
          lang: language,
          theme: selectedTheme,
          transformers: [
            transformerNotationHighlight({ matchAlgorithm: "v3" }),
        ...
        setHighlightedBefore(`<pre>${beforeCodeText}</pre>`);
        setHighlightedAfter(`<pre>${afterCodeText}</pre>`);
      }
    }
    highlightCode();
  }, [beforeCodeText, afterCodeText, language, selectedTheme]);
      ...
      <div className="group relative w-full overflow-hidden rounded-md border border-border">
        <div className="relative grid md:grid-cols-2">
          ...
          >
            ...
            {renderCode(beforeCodeText, highlightedBefore)}
          </div>
          <div
            className={cn(
          ...
          >
            ...
            {renderCode(afterCodeText, highlightedAfter)}
          </div>
        </div>
